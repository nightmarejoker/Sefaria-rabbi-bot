...
class SefariaClient:
    ...
    async def get_daily_text(self) -> Optional[Dict]:
        ...
        try:
            # Get calendar data
            calendar_data = await self._make_request("calendar")
            
            if not calendar_data:
                return None
            
            # Try to get the current Torah portion
            if 'calendar_items' in calendar_data:
                for item in calendar_data['calendar_items']:
                    if isinstance(item, dict) and 'title' in item:
                        if 'Parashat' in item['title'] or 'Torah' in item.get('category', ''):
                            # Get the text for this portion
                            ref = item.get('ref')
                            if ref:
                                return await self.get_text(ref)
            
            # Fallback to a default daily text
            return await self.get_text("Genesis 1:1")
